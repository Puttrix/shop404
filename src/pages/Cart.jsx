  ...
  return (
    <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-8">
      ...
      {state.items.length === 0 ? (
        <div className="p-6 border rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700">Your cart is empty. <Link className="text-brand-700" to="/products">Browse products</Link>.</div>
      ) : (
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-4">
            {state.items.map(item => (
              <div key={item.id+item.variant} className="flex items-center justify-between border rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700 p-4">
                <div>
